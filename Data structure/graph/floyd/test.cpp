...
int main(int argc, char const *argv[])
{
	...
	obj.print_param();
	std::cout << "\n" << "============================ BFS traverse ============================" << std::endl;
	obj.BFS();
	std::cout << "\n" << "============================ DFS traverse ============================"<< std::endl;
	obj.DFS();
	std::cout << "\n" << "============================ prim algorithm ==========================" << std::endl;
	obj.prim();
	std::cout << "\n" << "=========================== kruskal algorithm =========================" << std::endl;
	Edge ordered_edge[16] = {{0,1,7}, {2,4,1}, {3,4,2}, {6,7,2}, {1,2,3}, {4,5,3}, {3,6,3}, {7,8,4}, 
							 {0,2,5}, {1,4,5}, {5,7,5}, {4,6,6}, {1,3,7}, {2,5,7}, {6,8,7}, {4,7,9}};
	obj.kruskal(ordered_edge);
	int shorter_index[9];
	int shorter_length[9];
	std::cout << "\n" << "=========================== dijkstra algorithm =========================" << std::endl;
	std::cout << "shorter_index : " << std::endl;
	obj.dijkstra(shorter_length, shorter_index, 0);
	for (int i = 0; i < 9; ++i)
	{
		std::cout << "f(" << vertex_array[i] << ") = " << vertex_array[shorter_index[i]] << std::endl;
	}
	std::cout << "\n" << "shorter_length :" << std::endl;
	for (int i = 0; i < 9; ++i)
	{
		std::cout << "A" << vertex_array[i] << " = " << shorter_length[i] << std::endl;
	}
	int shorter_matrix[9][9];
	int shorter_num[9][9];
	obj.floyd(shorter_matrix, shorter_num);
	std::cout << "\n" << "============================== floyd algorithm ===========================" << std::endl;
	for (int i = 0; i < 9; ++i)
	{
		for (int j = 0; j < 9; ++j)
		{
			std::cout << vertex_array[i] << vertex_array[j] << "=" << shorter_matrix[i][j] << "   ";
		}
		std::cout << std::endl;
	}
	for (int i = 0; i < 9; ++i)
	{
		...
		for (int j = 0; j < 9; ++j)
		{
			std::cout << "f(" << vertex_array[j] << ")=" << vertex_array[shorter_num[i][j]] << "  ";
		}
		std::cout << std::endl;
	}
	return 0;
