...
int TextQuery::index_size() const
{
...
void TextQuery::print_result(set<TextQuery::line_no>& target_set, string word, TextQuery& check)
{
	string time_tail = target_set.size() > 1 ? "times" : "time";
	cout << endl << word << " occuers " << target_set.size() << " " << time_tail << endl << endl;
